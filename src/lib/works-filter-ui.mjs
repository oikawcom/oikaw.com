...
export function setupWorksFilter({ trayController } = {}) {
  ...
  if (filterRoots.length === 0 || !trayController) return null;

  const controllers = filterRoots.map((filterRoot) => {
  ...
  const update = (draft = trayController.getState().draft) => {
    for (const controller of controllers) controller.update(draft);
  };
  const updateScrollFade = () => {
    for (const controller of controllers) controller.updateScrollFade();
  };
