...
assert.ok(aboutSource.includes('action={contact.formspreeEndpoint}'));
assert.ok(aboutSource.includes('method="POST"'));
assert.ok(aboutSource.includes('name="What\'s this about?"'));
assert.equal(aboutSource.match(/name="email"/g)?.length, 1);
assert.ok(aboutSource.includes('name="message"'));
assert.ok(aboutSource.includes('name="_gotcha"'));
assert.ok(aboutSource.includes('cf-turnstile-response'));
assert.ok(aboutSource.includes('Accept: \'application/json\''));
assert.ok(aboutSource.includes('if (isSubmitting) return'));
assert.ok(aboutSource.includes("submitButton.disabled = true"));
assert.ok(aboutSource.includes("showStatus('Message sent.', 'success')"));
assert.ok(aboutSource.includes("showStatus('Couldn\\u2019t send. Please try again.', 'error')"));
assert.ok(aboutSource.includes('form.reset()'));
assert.ok(aboutSource.includes('resetTurnstile()'));
assert.ok(aboutSource.includes('api.js?onload=onContactTurnstileLoad&render=explicit'));
assert.ok(aboutSource.includes("appearance: 'interaction-only'"));
assert.ok(aboutSource.includes("window.turnstile.getResponse(widgetId)"));
assert.ok(aboutSource.includes("window.turnstile.reset(widgetId)"));
assert.ok(!aboutSource.includes('console.log'));
assert.ok(!aboutSource.includes('console.error'));
